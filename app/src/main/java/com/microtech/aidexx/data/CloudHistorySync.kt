...
abstract class CloudHistorySync<T : EventEntity> {
        ...
        if (needUploadData.size > 0) {
            ...
            withContext(Dispatchers.IO) {
                ...
                if (result.info.code == RESULT_OK) {
                    replaceEventData(needUploadData, result.content.records)
                } else {
                    downloadData()
                }
            }
    ...
    suspend fun downloadData(authorizationId: String? = null) {
        ...
        if (result.info.code == RESULT_OK) {
            replaceEventData(
                responseList = result.content.records,
                cgmStatus = 3,
                authorId = authorizationId
            ) //下载的数据
            val page = result.content.pageInfo
    ...
    suspend fun getNeedDeleteList(): MutableList<T>? {
        return ObjectBox.store.awaitCallInTx {
            val find = entityBox.query()
                .equal(deleteStatus, 1)
                .notNull(id)
                .equal(
                    authorizationId, UserInfoManager.instance().userId(),
        ...
        if (origin.isNotEmpty()) {
            ...
            for (res in responseList) {
                val existRecord = entityBox.query().equal(recordUuid, uuidValue(res) ?: "")
                    .equal(authorizationId, authorId ?: userId)
                    .build()
                    .find()
