...
class CalibrateEntity : BaseEventEntity() {
    ...
    override fun toString(): String {
        return "CalibrateEntity(userId = $userId,calTime=$calTime, language='$language', deviceId='$deviceId', eventIndex=$eventIndex, calibrationId=$calibrationId, sensorIndex=$sensorIndex, sensorId=$sensorId, referenceGlucose=$referenceGlucose, indexBeforeCal=$indexBeforeCal, indexAfterCal=$indexAfterCal, cf=$cf, offset=$offset, isValid=$isValid, index=$index, autoIncrementColumn=$autoIncrementColumn, timeOffset=$timeOffset,appTime=$appTime,appTimeZone=$appTimeZone,dstOffset=$dstOffset)"
    }
}