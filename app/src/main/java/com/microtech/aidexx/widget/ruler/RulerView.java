package com.microtech.aidexx.widget.ruler;
...
public class RulerView extends View {
    ...
    private void convertValue2Number() {
        setColorStartValues(colorStartValues);
...
//            logD("drawGradation: startNum=%d", startNum);
    ...
    private void drawColorBar(Canvas canvas) {
        if (mColorStartNumber==null || mColorStartNumber.length==0) {
            return;
        }
        ...
        // 当前绘制刻度对应控件左侧的位置
        float startPosition = mHalfWidth - (mCurrentDistance - (startNum - mMinNumber) / mNumberUnit * gradationGap);
        float endPosition = mHalfWidth - (mCurrentDistance - (rightMaxNum - mMinNumber) / mNumberUnit * gradationGap);
        for (int i=0; i<mColorStartNumber.length; i++) {
            float position;
            if (i == stickyIndex) {
                position = mHalfWidth;
            } else if (mColorStartNumber[i] < startNum) {
                position = startPosition;
            } else if (mColorStartNumber[i] <= mMaxNumber){
                position = mHalfWidth - (mCurrentDistance - (mColorStartNumber[i] - mMinNumber) / mNumberUnit * gradationGap);
            } else {
                position = endPosition;
            }
            mPaint.setColor(barColors[i% barColors.length]);
            canvas.drawLine(position, barWidth/2, endPosition, barWidth/2, mPaint);
        }
    }
    ...
    public void setColorStartValues(float[] colorStartValues) {
        this.colorStartValues = colorStartValues;
        if (colorStartValues!=null && colorStartValues.length>0) {
            mColorStartNumber = new int[colorStartValues.length];
            boolean defaultICC = indicatorColorChange==null || indicatorColorChange.length!=colorStartValues.length;
            if (defaultICC) {
                indicatorColorChange = new boolean[colorStartValues.length];
            }
            for (int i = 0; i < colorStartValues.length; i++) {
                mColorStartNumber[i] = (int) (colorStartValues[i] * 10);
                if (defaultICC) {
                    indicatorColorChange[i] = false;
                }
            }
        }
    }
    ...
    public void setIndicatorColorChange(boolean[] indicatorColorChange) {
        this.indicatorColorChange = indicatorColorChange;
    }
        ...
        if (mColorStartNumber!=null && mColorStartNumber.length>0) {
            for (int i = 0; i < mColorStartNumber.length; i++) {
                ...
                if (i==stickyIndex) {
                    ...
                    number = mColorStartNumber[i];
                }
                if (mCurrentNumber > number || (mCurrentNumber==number && indicatorColorChange[i])) {
                    indicatorLineColor = barColors[i % barColors.length];
                } else {
                    break;
                }
            }
