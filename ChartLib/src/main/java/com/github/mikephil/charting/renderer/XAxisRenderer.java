...
//        Utils.drawXAxisValue(c, formattedLabel, x, y, mAxisLabelPaint, anchor, angleDegrees);
        ...
        Paint secondPaint = new Paint(mAxisLabelPaint);
        secondPaint.setAlpha(128);
        ...
        if (labels.length > 1) {
            ...
            float xValue = x;
            float yValue = y;
            int styleBegin = labels[1].indexOf('{') + 1;
            int styleEnd = labels[1].lastIndexOf('}');
            if (styleBegin > 0 && styleEnd > styleBegin) {
                String style = labels[1].substring(styleBegin, styleEnd);
                labels[1] = labels[1].substring(styleEnd + 1);
                float xValueFix = mViewPortHandler.offsetLeft() + Utils.calcTextWidth(mAxisLabelPaint, labels[1]) / 2f;
                if (style.contains("f") || x < xValueFix) { xValue = xValueFix; }
