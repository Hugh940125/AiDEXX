 ...
 * Return Type  : void
 */
void MAXBG(const emxArray_real_T *sg, emxArray_real_T *maxbg)
{
  ...
  maxbg->size[0] = 1;
  maxbg->size[1] = varargout_1->size[1] + 1;
  emxEnsureCapacity_real_T(maxbg, n);
  i = varargout_1->size[1];
  for (n = 0; n < i; n++) {
    maxbg->data[maxbg->size[0] * n] = varargout_1->data[varargout_1->size[0] * n];
  }

  maxbg->data[maxbg->size[0] * varargout_1->size[1]] = mtmp;
  emxFree_real_T(&varargout_1);
}

/*
 * File trailer for MAXBG.c
 *
 * [EOF]
 */
