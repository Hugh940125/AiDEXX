...
//            Log.d("chart getEntryIndexForXValue", "mEntries.size=" + mEntries.size() + " cost=" + (System.currentTimeMillis() - start));
    ...
    @Override
    public List<T> getEntriesForXRange(float xMin, float xMax) {
        ...
        while (low <= high) {
            ...
            // if we have a match
            if (xMin <= entry.getX() && xMax >= entry.getX()) {
                while (m > 0 && xMin <= mEntries.get(m - 1).getX() && xMax >= mEntries.get(m - 1).getX())
                    m--;
