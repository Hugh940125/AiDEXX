...
public class LineDataSet extends LineRadarDataSet<Entry> implements ILineDataSet {
    ...
    public int getCircleColor(int index) {
        if (mCircleColors != null && mCircleColors.size()>0) {
    ...
    @Override
    public int getCircleColorIndex(Entry e) {
        if (mCircleColorRanges != null) {
            float y = e.getY();
            int i = 0;
            for (float position : mCircleColorRanges) {
                if (y > position) {
                    break;
                }
                i++;
    ...
    public void setCircleColorRanges(List<Float> circleColorRanges) {
        mCircleColorRanges = circleColorRanges;
    }
