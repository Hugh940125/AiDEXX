...
public class LineChartRenderer extends LineRadarRenderer {
    ...
    protected void drawCubicBezier(ILineDataSet dataSet) {
        ...
        mRenderPaint.setShader(generateLinearGradient(dataSet, dataSet.getColors(), dataSet.getGradientPositions()));
        mBitmapCanvas.drawPath(cubicPath, mRenderPaint);
    ...
    protected void drawCubicFill(Canvas c, ILineDataSet dataSet, Path spline, Transformer trans, XBounds bounds) {
        final ILineDataSet fillMinDataSet = dataSet.getFillFormatter().getFillLine(dataSet, mChart);

        if (fillMinDataSet != null) {
            XBounds fillMinXBounds = new XBounds();
            fillMinXBounds.set(mChart, fillMinDataSet);

            addFillMinPath(spline, fillMinDataSet, fillMinXBounds);
        } else {
    ...
    protected void drawLinearFill(Canvas c, ILineDataSet dataSet, Transformer trans, XBounds bounds) {
        ...
        generateFilledPath(dataSet, startingIndex, endingIndex, filled);
...
//        } while (currentStartIndex <= currentEndIndex);
    ...
    private void generateFilledPath(final ILineDataSet dataSet, final int startIndex, final int endIndex, final Path outputPath) {
        ...
        Entry previousEntry = startEntry;
        ...
        if (currentEntry != null) {
            if (fillMinDataSet != null) {
                ...
                filled.lineTo(startEntry.getX(), fillMin);
            }
        }
    ...
    private void addFillMinPath(Path filled, final ILineDataSet dataSet, XBounds xBounds) {
        final float phaseY = mAnimator.getPhaseY();

        switch (dataSet.getMode()) {
            case LINEAR:
            case STEPPED:
                final int startIndex = xBounds.min;
                final int endIndex = xBounds.range + xBounds.min;

                final boolean isDrawSteppedEnabled = dataSet.getMode() == LineDataSet.Mode.STEPPED;

                Entry currentEntry = dataSet.getEntryForIndex(endIndex);
                if (currentEntry == null) {
                    return;
                }
                ...
                Entry previousEntry = currentEntry;
            ...
            case CUBIC_BEZIER:
                ...
                if (xBounds.range >= 1) {
                    ...
                    int maxIndex = dataSet.getEntryCount() - 1;
                    Entry prevPrev;
                    Entry prev = dataSet.getEntryForIndex(Math.min(lastIndex + 2, maxIndex));
                    Entry cur = dataSet.getEntryForIndex(Math.min(lastIndex + 1, maxIndex));
                    Entry next = cur;
                    int nextIndex = 0;
                    ...
                    for (int j = lastIndex; j >= firstIndex - 1; j--) {
                        ...
                        filled.cubicTo(prev.getX() + prevDx, (prev.getY() + prevDy) * phaseY,
                                cur.getX() - curDx,
                                (cur.getY() - curDy) * phaseY, cur.getX(), cur.getY() * phaseY);
                    }
                }
...
//
    ...
    protected void drawCircles(Canvas c) {
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            for (int j = mXBounds.min; j <= boundsRangeCount; j++) {
                ...
                Bitmap circleBitmap = imageCache.getBitmap(dataSet.getCircleColorIndex(e));
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (Highlight high : indices) {
            ...
            mRenderPaint.setColor(set.getCircleColor(set.getCircleColorIndex(e)));
            mRenderPaint.setStyle(Paint.Style.FILL);
            c.drawCircle((float) pix.x, (float) pix.y, set.getCircleRadius() * 1.75f, mRenderPaint);
        }
    }
    ...
    private LinearGradient generateLinearGradient(ILineDataSet dataSet, List<Integer> colors, List<Float> positions) {
        ...
        double maxY = mChart.getTransformer(dataSet.getAxisDependency()).getValuesByTouchPoint(
                mViewPortHandler.contentRight(), mViewPortHandler.contentTop()).y;
        double minY = mChart.getTransformer(dataSet.getAxisDependency()).getValuesByTouchPoint(
                mViewPortHandler.contentLeft(), mViewPortHandler.contentBottom()).y;

        for (float position : positions) {
            positionArray[i] = (float) ((maxY - position) / (maxY - minY));
            i++;
        }

        return new LinearGradient(
                0, mChart.getContentRect().top, 0, mChart.getContentRect().bottom,
        ...
        protected void fill(ILineDataSet set, boolean drawCircleHole, boolean drawTransparentCircleHole) {
            ...
            for (int i = 0; i < colorCount; i++) {
                ...
                if (drawTransparentCircleHole) {
                    ...
                    if (drawCircleHole && circleRadius > 0 && circleHoleRadius > 0
                            && mCirclePaintInner != null) {
